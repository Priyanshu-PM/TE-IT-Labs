...
void calc_profit(int capacity, item items[], int n, int type){
    ...
    for(int i=0; i<n; i++){
        if(capacity - items[i].weight >= 0){
            ...
            cout << prd(items[i].weight, 0, 8) << " | " << prd(items[i].profit, 0, 15) << " | " <<prd(total_profit, 2, 10) << "\n";
        
        }else{
            if(type == 1){
                total_profit += (capacity/items[i].weight) * items[i].profit;
                string str =  (capacity>0) ? "yes - original weight= "+to_string(items[i].weight): "no";
                cout << prd(capacity, 0, 8) << " | " << prd(items[i].profit, 0, 15) << " | " <<prd(total_profit, 2, 10) << " | Picked ?" << str << "\n";
                capacity = 0; 
            }
            if(capacity == 0) break;
        }
